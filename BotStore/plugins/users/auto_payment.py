    ...
    while time.time() - start_time < (
        ...
        if rt == "PAGO":
           ## rt = "PAGO"
            ...
            await c.send_message(
                GRUPO_PUB,
                f"<a href='https://t.me/Lonsepo'>&#8204</a>💰 {mention} Adicionou <b>R${value}</b> de saldo no bot\n\n<a href='https://t.me/Lonsepo'>Suporte</a>",
            )
...
async def auto_pay(c: Client, m: Union[CallbackQuery, Message]):
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="✔️ 00:00:30 !Aguarde!", callback_data="add_saldo_auto"
                )
            ]
        ]
    )
    ...
    if (call_function(f"{id_flood}") == True):
            return await m.reply_text(
                f"<b>⚠️ Aguarde 30 segundos para usar o /pix novamente!</b>", reply_markup=kb
            )
...
⚠️ Caso não consiga efetuar o pagamento, chame o 👉 @Lonsepo</b>""",
...
⚠️ Se não conseguir fazer pix automático, chame o: @Lonsepo
"""
