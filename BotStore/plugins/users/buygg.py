...
@Client.on_callback_query(filters.regex(r"^comprar_gg$"))
async def comprar_cc_list(c: Client, m: CallbackQuery):
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton("💳 Unitária", callback_data="comprar_gg unit"),
            ],
            [
                InlineKeyboardButton(
                    "🏦 Buscar banco",
                    switch_inline_query_current_chat="buscargg_banco BANCO",
                ),
                InlineKeyboardButton(
                    "🔐 Buscar bin",
                    switch_inline_query_current_chat="buscargg_bin 550209",
                ),
            ],
            [
                InlineKeyboardButton(
                    "🏳️ Buscar bandeira",
                    switch_inline_query_current_chat="buscargg_bandeira MASTERCARD",
                ),
            ],
[
                InlineKeyboardButton(
                    "🇧🇷 Buscar países",
                    switch_inline_query_current_chat="buscargg_paises BR",
                ),
            ],

             
            [
                InlineKeyboardButton("❮", callback_data="comprar_log"),
            ],
        ]
    )
...
@Client.on_inline_query(filters.regex(r"^buscargg_(?P<type>\w+) (?P<value>.+)"))
async def search_cc(c: Client, m: InlineQuery):
    ...
    rt = cur.execute(
        f"SELECT number, month, year, {typ2}, country FROM ggs WHERE {typ2} LIKE ? AND pending = 0 ORDER BY RANDOM() LIMIT 50",
        [qry.upper()],
    ).fetchall()
...
@Client.on_callback_query(filters.regex(r"^comprar_gg unit$"))
async def comprar_ccs(c: Client, m: CallbackQuery):
    ...
    for level in levels_list:
        ...
        levels.append(
            InlineKeyboardButton(
                ...
                callback_data=f"comprar_gg unit '{level}'",
            )
        )
    ...
    organ.append([InlineKeyboardButton(text="❮ ❮", callback_data="comprar_gg")])
...
@Client.on_callback_query(
    filters.regex(
        r"^comprar_gg (?P<type>[a-z]+) '(?P<level_cc>.+)' ?(?P<other_params>.+)?"
    )
)
@lock_user_buy
async def buy_final(c: Client, m: CallbackQuery):
    ...
    ccs_list = cur.execute(
        f"SELECT number, month, year, cvv, level, added_date, vendor, bank, country, cpf, name FROM ggs WHERE {search_for} = ? AND pending = ? ORDER BY RANDOM() LIMIT 20",
        [level_cc, False],
    ).fetchall()

    if not ccs_list:
        return await m.answer("⚠️ Não há GGs disponíveis deste nível.")
    ...
    if gates_is_on:
        ...
        for tp in ccs_list:
            ...
            is_pending = cur.execute(
                "SELECT pending FROM ggs WHERE number = ?", [tp[0]]
            ).fetchone()
            ...
            if not is_pending or is_pending[0]:
                continue
            cur.execute("UPDATE ggs SET pending = 1 WHERE number = ?", [tp[0]])
            ...
            if live_or_die[0]:  # caso venha cc live
                ...
                cur.execute(
                    "DELETE FROM ggs WHERE number = ?",
                    [tp[0]],
                )

                insert_buy_sold(list_dados)
                insert_sold_balance(price, user_id, "ggs")

                kb = InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text="🔕 Não passou? Tente Seu reembolso de Saldo",
                ...
                kb = InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text="💳 Compre as melhores ggs",url=f"https://t.me/FenixLoginsBot"
                            ),
                        ],
                    ]
                )
                ...
                cur.execute(
                    "UPDATE ggs SET pending = False WHERE number = ?", [tp[0]]
                )
                ...
                cur.execute(
                    f"INSERT INTO ggs_dies({values}) VALUES(?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                    list_dies,
                )

        if (live == 0) and gates_is_on:
            txt = "❗️ Não consegui achar GGs lives deste nível na minha database. Tente novamente com outro nível ou bin."
            kb = InlineKeyboardMarkup(
                inline_keyboard=[
                    [
                        InlineKeyboardButton(
...
async def buy_off(c: Client, m: CallbackQuery):
    ...
    selected_cc = cur.execute(
        f"SELECT number, month, year, cvv, level, added_date, vendor, bank, country, cpf, name FROM ggs WHERE {search_for} = ? AND pending = ? ORDER BY RANDOM()",
        [level_cc, False],
    ).fetchone()

    if not selected_cc:
        return await m.answer("⚠️ Sem ggs disponiveis para este nivel.", show_alert=True)
