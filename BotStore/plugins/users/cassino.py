...
async def dicegames(c: Client, m: Message):
        ...
        [
        InlineKeyboardButton("❌ Cancelar ", callback_data="cassino"),
        ],
        ]
        )
...
async def stardices(c: Client, m: Message, CallbackQuery):
    user_id = m.from_user.id
    price = 6
...
async def boliches(c: Client, m: Message):
        ...
        inline_keyboard=[
        [
        InlineKeyboardButton("✅ Continue",callback_data="jogarboliche"),
        ],
        [
        InlineKeyboardButton("❌ Cancelar ", callback_data="cassino"),
        ],
...
@Client.on_callback_query(filters.regex("^jogarboliche$"))
