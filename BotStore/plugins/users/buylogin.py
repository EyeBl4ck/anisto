...
async def comprar_logins(c: Client, m: CallbackQuery):
    ...
    total = f"<b>🧿 Total de Produtos</b>: {sum([int(x[1]) for x in logins])}" if logins else ""
    
    organ.append([InlineKeyboardButton(text="⬅️ Voltar", callback_data="comprar_contas")])
    kb = InlineKeyboardMarkup(inline_keyboard=organ)
    await m.edit_message_text(
        f"""<b>🎟 Adquira abaixo seu produto de preferência.</b>
...
async def buy_off_logins(c: Client, m: CallbackQuery):
    ...
    kb = InlineKeyboardMarkup(
                    inline_keyboard=[
                        [
                            InlineKeyboardButton(
                                text="Acessar Loja",url=f"https://t.me/BetanoLojinha_bot"
                            ),
                        ],
                    ]
                )
    ...
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton(text="⬅️ Voltar", callback_data="comprar_contas"),
            ],
        ]
    )
    try:
