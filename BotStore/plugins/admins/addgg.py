    ...
    for row in re.finditer(
        ...
        if info:
            ...
            available = cur.execute(
                "SELECT added_date FROM ggs WHERE number = ?", [row["number"]]
            ).fetchone()
            solds = cur.execute(
                "SELECT bought_date FROM ggs_sold WHERE number = ?",
                [row["number"]],
            ).fetchone()
            dies = cur.execute(
                "SELECT die_date FROM ggs_dies WHERE number = ?",
                [row["number"]],
            ).fetchone()
            ...
            cur.execute(
                "INSERT INTO ggs(number, bin, month, year, cvv, vendor, level, bank, country, card_type, cpf, name) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
                (
                    row["number"],
                    card_bin,
                    row["month"],
...
@Client.on_message(filters.regex(r"/gg( (?P<cards>.+))?", re.S) & filters.user(ADMINS))
async def on_add_m(c: Client, m: Message):
    ...
    if cards:
        ...
        if not total:
            text = (
                "❌ Não encontrei GGs na sua mensagem. Envie elas como texto ou arquivo."
            )
        else:
            text = f"✅ {success} GGs adicionadas com sucesso. Repetidas/Inválidas: {(total - success)}"
    ...
    await m.reply_text(
        "💳 Modo de adição ativo. Envie as GGs como texto ou arquivo e elas serão adicionadas.",
    ...
    while True:
        if not first:
            await m.reply_text(
                "✅️ Envie mais GGs ou digite /done para sair do modo de adição.",
                reply_markup=ForceReply(),
            )
        ...
        if not msg.text and (
            ...
            await msg.reply_text(
                "❕ Eu esperava um texto ou documento contendo as GGs.", quote=True
            )
            continue
        ...
        if open("ggs.txt").read() != "":
            await sent.reply_document(open("ggs.txt", "rb"), quote=True)
        os.remove("ggs.txt")

    await m.reply_text(
        "✅ Você Saiu do modo de adição de GGs.", reply_markup=ReplyKeyboardRemove()
    )
