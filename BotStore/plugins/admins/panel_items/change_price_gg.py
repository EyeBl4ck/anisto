...
@Client.on_callback_query(filters.regex(r"^change_price_gg$") & filters.user(ADMINS))
async def change_prices1(c: Client, m: CallbackQuery):
    kb = InlineKeyboardMarkup(
        inline_keyboard=[
            [
                InlineKeyboardButton("💳 Unitario", callback_data="change_price unit"),
                InlineKeyboardButton("💳 UNITARIO", callback_data="change_price_gg gg"),
...
@Client.on_callback_query(
    filters.regex(r"^change_price_gg (?P<ptype>.+)") & filters.user(ADMINS)
)
