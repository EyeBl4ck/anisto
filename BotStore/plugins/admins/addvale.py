...
async def on_add_m(c: Client, m: Message):
    ...
    if cards:
        ...
        if not total:
            text = (
                "❌ Não encontrei DADOS na sua mensagem. Envie eles como texto ou arquivo."
            )
        else:
            text = f"✅ {success} DADOS adicionados com sucesso. Repetidos/Inválidos: {(total - success)}"
    ...
    await m.reply_text(
        """💳 Modo de adição ativo. Envie os DADOS como texto ou arquivo e elas serão adicionadas. (Na parte "IND", coloca qualquer letra ou número) Caso não for, retire as "/" do nascimento.
TIPO|NOME|TELEFONE|CPF|NASCIMENTO|IND

EXEMPLO: DADOS CPF|Ribeiro Silva Augusto|22987654321|12345678998|14/05/2002|SP""",
    ...
    while True:
        if not first:
            await m.reply_text(
                "✅ Envie mais DADOS ou digite /done para sair do modo de adição.",
                reply_markup=ForceReply(),
            )
        ...
        if not msg.text and (
            ...
            await msg.reply_text(
                "❕ Eu esperava um texto ou documento contendo as DADOS.", quote=True
            )
            continue
        ...
        if not total:
            text = (
                "❌ Não encontrei DADOS na sua mensagem. Envie elas como texto ou arquivo."
            )
        else:
            text = f"✅ {success} DADOS adicionados com sucesso. Repetidos/Inválidos: {(total - success)}"
